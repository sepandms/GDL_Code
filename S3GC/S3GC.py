...
reddit_params   = {'walk_length':5 , 'hidden_dims':256, 'walks_per_node':20 , 'batch_size':256 , 'lr':0.005,'epochs':40,'log_steps':100}
...
embedding_store = f'S3GC_embedding_{dataset_name}.pt'
...
def main():
    ...
    print(' ############### Final Results on  vector embedding clustering ###############') 
    embedding = torch.load(embedding_path + embedding_store)
    # y_pred = kmeans.fit_predict(embedding.detach().cpu().numpy())
    kmeans = clone(best_kmeans)
    cm = clustering_metrics(y.cpu().numpy(), y_pred)
    acc, nmi, cs, f1_macro, adjscore  = cm.get_main_metrics()
    ...
    plt.title(f'S3GC: Performance Metrics ({dataset_name})')
    plot_name = f'S3GC_{dataset_name}.png'
    if save_plots:
